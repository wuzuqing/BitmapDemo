...
//               LogUtils.logd("nodeInfo: " + nodeInfo);
            }
    ...
    private boolean performClick(List<AccessibilityNodeInfo> nodeInfos) {
        LogUtils.logd("nodeInfos:" + nodeInfos);
        if (nodeInfos != null && !nodeInfos.isEmpty()) {
            AccessibilityNodeInfo node;
            for (int i = 0; i < nodeInfos.size(); i++) {
                node = nodeInfos.get(i);
                // 获得点击View的类型
                LogUtils.logd("View类型：" + node.getClassName());
                // 进行模拟点击
                if (node.isEnabled()) {
    ...
    public void inputText(AccessibilityNodeInfo target, String text) {
        //模拟点击事件
    ...
    /**根据固定的类名循环查找到输入框,查找目标界面的所有输入框，适用于界面有好多输入框的情况
     * @param editTexts 传入一个size为0的list。
     * @param node
     */
    public void recycleFindEditTexts(List<AccessibilityNodeInfo> editTexts, AccessibilityNodeInfo node) {
        //edittext下面必定没有子元素，所以放在此时判断
        if (node.getChildCount() == 0) {
            if (editTextClassName.equals(node.getClassName())) {
                LogUtils.logd("查找到一个输入框。。。");
                editTexts.add(node);
            }
        } else {
            for (int i = 0; i < node.getChildCount(); i++) {
                recycleFindEditTexts(editTexts,node.getChild(i));
            }
        }
    }
